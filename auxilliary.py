...
class BuildTree():
    def __init__(self, giveParameters = ['isRoot', {}, 0, 0, []] ): #add init dist
        ...
        for i in giveParameters:
            if type(i) == str or type(i) == int or type(i) == list: #OrHaim added or type(i) == list
    ...
    def make_new_root(self, new_root):
        self.nodes[-1] = self.nodes[new_root][:]#OrHaim changed, original was: self.nodes[-1] = self.nodes[new_root].copy()
