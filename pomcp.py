...
class POMCP():
    ...
    def __init__(self, generator, gamma = 0.95, c = 1, threshold = 0.005, timeout = 100, no_particles = 1200):
        self.gamma = gamma 
        if gamma >= 1:
    ...
    # Search module
    def Search(self):
        Bh = self.tree.nodes[-1][4][:] #OrHaim original:  Bh = self.tree.nodes[-1][4].copy()
    ...
    def UpdateBelief(self, action, observation):
        prior = self.tree.nodes[-1][4][:]#OrHaim changed: original: prior = self.tree.nodes[-1][4].copy()
